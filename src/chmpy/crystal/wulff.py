...
def safe_divide(a, b):
    tiny = np.finfo(float).eps  # Use machine epsilon to avoid zero division
    return np.divide(a, np.where(b == 0, tiny, b))
...
def expand_symmetry_related_planes(hkl, energies, spacegroup):
    ...
    for i, s in enumerate(spacegroup.symmetry_operations):
        expanded_facets[i * nfacets : (i + 1) * nfacets] = (hkl @ s.rotation.T).astype(
            int
        )
    ...
    for i, x in enumerate(expanded_facets):
        ...
        # if (
        #     reduced_neg not in unique
        #     or tiled_energies[unique[reduced_neg]] > tiled_energies[i]
        # ):
...
def project_to_plane(points, plane_normal):
    ...
    if projected_points.shape[0] < 2:
        raise ValueError(
            "Insufficient points for projection. Caused by projected points: {projected_points} from pts: {points}."
        )
...
class WulffConstruction:
    ...
    def _populate_duals(self):
        ...
        self.facet_dual_vectors = safe_divide(
    ...
    def _extract_wulff_from_dual_mesh(self):
        ...
        scaling_factors = safe_divide(corresponding_facet_energies, inv_factors)
